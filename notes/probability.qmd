...
When $X$ is _continuous_ it doesn't make sens to have events like $X = v$, rather we use $v \leq X \leq w$. In this case we work with probability **densities**. An example would be the height of a population.

We note $\operatorname{P}(X)$ the **probability distribution** of X.

#### Multiple Random Variables

$\operatorname{P}(A = a, B = b)$ is the **joint probability** of $A = a$ **and** $B = b$ (it's the intersection of the events $A = a$ and $B = b$).

Obviously $$ \operatorname{P}(A = a, B = b) \leq \operatorname{P}(A=a) \quad \text{and} \quad \operatorname{P}(A = a, B = b) \leq \operatorname{P}(B=b) $$

Also, $$ \operatorname{P}(A = a) = \sum_v \operatorname{P}(A = a, B = v) $$
...
* likelihood: a mesure of how compatible our hypothesis is with our observation
...
* $\mathcal{A} \subset \mathcal{B}$, $\mathcal{A} \subsetneq \mathcal{B}$: $\mathcal{A}$ is a proper/strict subest of $\mathcal{B}$
...
* $\mathcal{A} \setminus \mathcal{B}$: set substraction of $\mathcal{B}$ from $\mathcal{A}$, elements from $\mathcal{A}$ but not in $\mathcal{B}$
