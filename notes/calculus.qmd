...
The **partial derivative** of $y$ with respect to its $i^\textrm{th}$ parameter $x_i$ is
...
The **Jacobian** $\mathbf{J} \in \mathbb{R}^{m \times n}$ is a generalization of the [gradient @sec-gradient] to $\mathbf{y} = f(\mathbf{x})$ with $f \colon \mathbb{R}^n \to \mathbb{R}^m$, where $j_{i,j} = \dfrac{\partial y_i}{\partial\mathbf{x}_j}$, $\mathbf{x} = \begin{bmatrix} x_1 \\ \vdots \\ x_n \end{bmatrix} \in \mathbb{R}^n$ and $\mathbf{y} = \begin{bmatrix} y_1 \\ \vdots \\ y_m \end{bmatrix} \in \mathbb{R}^m$.

Explicitly
$$ \displaystyle{ \mathbf{J} = \begin{bmatrix} \dfrac{\partial \mathbf{y}}{\partial x_{1}} & \cdots & \dfrac{\partial \mathbf{y}}{\partial x_{n}} \end{bmatrix}
